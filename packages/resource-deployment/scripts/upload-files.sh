...
uploadFolderContents() {
    ...
    az storage blob upload-batch --account-name "$storageAccountName" --destination "$destinationContainer" --source "$pathToSource" --pattern "$includePattern" --overwrite=true --auth-mode login 1>/dev/null
}
...
uploadFile() {
    ...
    az storage blob upload --account-name "$storageAccountName" --container-name "$destinationContainer" --file "$pathToSource" --name "$blobName" --overwrite=true --auth-mode login 1>/dev/null
}
