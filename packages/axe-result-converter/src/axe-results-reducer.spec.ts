...
describe(AxeResultsReducer, () => {
    ...
    it('reduce result without nodes', () => {
        ...
        const accumulatedResults = addAxeResult(
            new AxeResultsList(),
            getAccumulatedResult('rule-1', { urls: ['url-1'], urlInfos: [{ url: 'url-1' }] }),
        );
        const currentResults = getCurrentResults('rule-1');
        const expectedResults = [
            getAccumulatedResult('rule-1', { urls: ['url-1', currentUrl], urlInfos: [{ url: 'url-1' }, { url: currentUrl }] }),
        ];
    ...
    it('skip same rule node', () => {
        ...
        const expectedResults = addAxeResult(
            ...
            getAccumulatedResult('rule-1', {
                ...
                urlInfos: [{ url: 'url-1' }, { url: currentUrl }],
                nodeId: 'node-1',
            }),
