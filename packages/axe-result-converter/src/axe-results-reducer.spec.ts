...
describe(AxeResultsReducer, () => {
    ...
    it('reduce axe result', () => {
        ...
        const expectedViolations = [getAccumulatedResult('rule-1', { urls: [url], urlInfos: [], nodeId: 'node-11' })];
        const expectedPasses = [getAccumulatedResult('rule-2', { urls: [url], urlInfos: [], nodeId: 'node-21' })];
        const expectedIncomplete = [getAccumulatedResult('rule-3', { urls: [url], urlInfos: [], nodeId: 'node-31' })];
        const expectedInapplicable = [getAccumulatedResult('rule-4', { urls: [url], urlInfos: [] })];
    ...
    it('reduce result without nodes', () => {
        ...
        const accumulatedResults = addAxeResult(new AxeResultsList(), getAccumulatedResult('rule-1', { urls: ['url-1'], urlInfos: [] }));
        const currentResults = getCurrentResults('rule-1');
        const expectedResults = [getAccumulatedResult('rule-1', { urls: ['url-1', currentUrl], urlInfos: [] })];
    ...
    it('skip same rule node', () => {
        ...
        const accumulatedResults = addAxeResult(
            new AxeResultsList(),
            getAccumulatedResult('rule-1', { urls: ['url-1'], urlInfos: [], nodeId: 'node-1' }),
        );
        const currentResults = getCurrentResults('rule-1', 'node-1', 'node-2', 'node-3');
        const expectedResults = addAxeResult(
            new AxeResultsList(),
            getAccumulatedResult('rule-1', {
                urls: ['url-1', currentUrl],
                urlInfos: [],
                nodeId: 'node-1',
            }),
            getAccumulatedResult('rule-1', { urls: [currentUrl], urlInfos: [], nodeId: 'node-2' }),
            getAccumulatedResult('rule-1', { urls: [currentUrl], urlInfos: [], nodeId: 'node-3' }),
        );
    ...
    it('split multiple nodes from a single result', () => {
        ...
        const expectedResults = addAxeResult(
            new AxeResultsList(),
            getAccumulatedResult('rule-1', { urls: [currentUrl], urlInfos: [], nodeId: 'node-1' }),
            getAccumulatedResult('rule-1', { urls: [currentUrl], urlInfos: [], nodeId: 'node-2' }),
            getAccumulatedResult('rule-1', { urls: [currentUrl], urlInfos: [], nodeId: 'node-3' }),
        );
