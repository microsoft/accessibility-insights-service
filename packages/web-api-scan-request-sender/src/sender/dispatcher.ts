...
export class DDispatcher {
    ...
    public async dispatchOnDemandScanRequests(): Promise<void> {
        this.logger.logInfo('Starting the dispatch');
        console.log('Happy Happy dispatcher');
    }
    ...
    //     do {
    //         do {
    //             const response: CosmosOperationResponse<WebsitePage[]> = await this.pageScanRequestProvider.getReadyToScanPages(
    //                 continuationToken,
    //             );
