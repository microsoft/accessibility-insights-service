...
import { ItemType, OnDemandPageScanRequest, PartitionKey, ReportGeneratorRequest } from 'storage-documents';
...
export class ReportGeneratorRequestProvider {
    ...
    public async insertRequests(requests: ReportGeneratorRequest[]): Promise<void> {
        return this.cosmosContainerClient.writeDocuments(requests, PartitionKey.reportGeneratorRequestDocuments);
    }
    ...
    public async deleteRequests(ids: string[]): Promise<void> {
        ...
        await Promise.all(
            ids.map(async (id) => {
                return limit(async () => this.cosmosContainerClient.deleteDocument(id, PartitionKey.reportGeneratorRequestDocuments));
            }),
        );
    }
}
